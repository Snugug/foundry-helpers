...
export function addToggleClasses(html) {
  if (html) {
    const sections = [...html.find('.main-controls > [data-control]')].map(
      (c) => c.dataset.control,
    );
    const subsections = [...html.find(`.sub-controls`)];

    for (let i = 0; i < subsections.length; i++) {
      const section = sections[i];
      const subsection = subsections[i];
      const controls = $(subsection).find('[data-tool]');
    ...
    const selected = [...document.querySelectorAll(`.cc--selected`)]
      .map((elem) => {
        if (elem.classList.contains('control-tool')) {
          return `.control-tool[data-tool="${elem.dataset.tool}"][data-section="${elem.dataset.section}"]`;
